...
values(category_l_seq.nextval, 'IT', '이것은 IT다', '', '', 0);

--카테고리(중)
insert into category_m
values(category_m_seq.nextval, (select category_no from category_l where category_name_l='IT'),
'자바', '잘하는 놈','','',0); 

--상품
insert into menu_info(PRODUCT_NO,  CATEGORY_TYPE_NO,  PRODUCT_NAME,  EVAL_SCORE,
  SUMMARY,  DETAIL_DESC,  ORDER_LEVEL, EXPERT_NO)
values(menu_info_seq.nextval, (select category_no_m from category_m where category_name='자바'),
