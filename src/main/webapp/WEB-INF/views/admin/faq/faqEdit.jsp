	...
	$(function(){
		$('#cancle').click(function(){
			if(confirm("수정 취소하시겠습니까?")){
				location.href="<c:url value='/admin/faq/faqDetail.do?faqNo=${vo.faqNo }'/>";
			}
		});
	});
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="basic-form">
                                        <form name="frmWrite" method="post" action="<c:url value='/admin/faq/faqEdit.do'/>">
                                        	<fieldset>
												<legend></legend>
	                                            <div class="form-group">
	                                            	<select name="faqCateNo" id="faqCateNo" class="form-control">
														...
														<c:forEach var="catvo" items="${list }" >
															...
															<c:if test="${catvo.faqCateNo == vo.faqCateNo }">
																selected
															</c:if>
															>${catvo.faqName }</option>
														</c:forEach>
													</select><br>
						            				<input type="text" id="faqTitle" name="faqTitle" class="form-control input-default" 
						            				maxlength="100" placeholder="제목을 입력하세요" required="required" value="${vo.faqTitle }"/>
	                                            </div>
	                                            
										        <div class="form-group">  
										 			<textarea id="faqContent" name="faqContent" class="form-control" style="width: 100%">${vo.faqContent }</textarea>
										        </div>
												<script type="text/javascript">
													//스마트에디터2 생성 코드
													var oEditors = [];
